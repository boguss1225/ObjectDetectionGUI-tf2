			...
			for (int i = 1; i < numofFolders; i++) {
				if (foldername[i].substring(2).startsWith(".")) {
					...
					if (foldername[i].equals("./test") || foldername[i].contains("train")
							...
							|| foldername[i].contains("test_images") || foldername[i].contains("test_videos")) {
						...
						fileBtns[numofvisiblefile].addMouseListener(new MouseAdapter() {
							...
							public void mousePressed(final MouseEvent e) {
								...
								//test forbidden files
										...
										||fileName.endsWith(".pb")||fileName.endsWith(".mov")||fileName.endsWith(".avi")
										||fileName.endsWith(".mp4")||fileName.endsWith(".wmv")||fileName.endsWith(".flv")){
									try {
